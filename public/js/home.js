    ...
    fetch(apiUrl)
        ...
        .then(async data => {
            if (data.length === 0) {
                ...
                //     num_1 = Math.floor(Math.random() * histori.length)

                //     if (num_1 in hist_rand_1){
                //         num_1 = Math.floor(Math.random() * histori.length)
                //     };
                ...
                function getUniqueRandomNumbers(count, range) {
                ...
                const obj_key = Object.keys(data_artist);
                
                const uniqueRandomNumbers = getUniqueRandomNumbers(5, histori.length);
                
                uniqueRandomNumbers.forEach((num, i) => {
                    let rec_img = document.getElementById(`rec-img-${i + 1}`);
                    let rec_p = document.getElementById(`rec-p-${i + 1}`);
                    let rec_but = document.getElementById(`rec-but-${i + 1}`);
                
                    let pop_img = document.getElementById(`pop-img-${i + 1}`);
                    let pop_p = document.getElementById(`pop-p-${i + 1}`);
                    let pop_but = document.getElementById(`pop-but-${i + 1}`);
                
                    let art_img = document.getElementById(`art-img-${i + 1}`);
                    let art_p = document.getElementById(`art-p-${i + 1}`);
                    let art_but = document.getElementById(`art-but-${i + 1}`);
                
                    let random_1 = histori[num];
                    let random_2 = histori[Math.floor(Math.random() * histori.length)];
                    let random_art = data_artist[obj_key[Math.floor(Math.random() * obj_key.length)]][0]['artists'][0];
